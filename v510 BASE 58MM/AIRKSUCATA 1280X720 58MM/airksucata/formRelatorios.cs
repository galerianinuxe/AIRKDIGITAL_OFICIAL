...
namespace airksucata
{
    public partial class formRelatorios : Form
    {
        ...
        private void btnConsultar_Click(object sender, EventArgs e)
        {
            ...
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                try
                {
                    ...
                    else if (radioTudo.Checked)
                    {

                        dataInicial = DateTime.Now.AddDays(-99999);
                        dataFinal = DateTime.Now;
                    }
                    else
                    {
        ...
        {
            DialogResult result = MessageBox.Show("Confirmar impressão do relatório completo?", "Cnfirmar Impressão", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (result == DialogResult.Yes)
            {
                // O usuário confirmou, então proceda com a impressão
        ...
        private void ImprimirRelatorioCompra(object sender, PrintPageEventArgs e)
        {
            ...
            formatoCentralizado.Alignment = StringAlignment.Near; // Alinhamento horizontal à esquerda
            formatoCentralizado.LineAlignment = StringAlignment.Center; // Centralizado verticalmente


            SizeF tamanhoCabecalho = e.Graphics.MeasureString(cabecalhoCompleto, fonteCabecalho);

            // Desenhar a imagem no topo centralizado
            int posicaoXImagem = (e.PageBounds.Width - 50) / 4; // Posição X centralizada
            ...
            int[] largurasColunas = { 15, 50, 40, 40, 45, 0 }; // Ajuste conforme necessário
            ...
            Font contatoFonte = new Font("Arial", 11, FontStyle.Bold);
            // Imprimir informações adicionais
            linhaAtual += margemInterna;
            string textoTotalCompra = $"Total Compras: \n{lbTotalCompra.Text}\n";
            string textoTotalPeso = $"Peso: \n{lbTotalPeso.Text}\n";
            string textoMaisComprado = $"Mais Comprado: \n{lbMaisComprado.Text}\n";
            string textoTotalPesoMaisComprado = $"Peso Mais Comprado:\n{lbTotalPesoMaterialMaisComprado.Text}\n\n";
            ...
            e.Graphics.DrawString(textoTotalCompra, contatoFonte, Brushes.Black, new Point(margemInterna, linhaAtual));
            linhaAtual += alturaTextoAdicional;
            linhaAtual += alturaTextoAdicional;
            e.Graphics.DrawString(textoTotalPeso, contatoFonte, Brushes.Black, new Point(margemInterna, linhaAtual));
            linhaAtual += alturaTextoAdicional;
            linhaAtual += alturaTextoAdicional;
            e.Graphics.DrawString(textoMaisComprado, contatoFonte, Brushes.Black, new Point(margemInterna, linhaAtual));
            linhaAtual += alturaTextoAdicional;
            linhaAtual += alturaTextoAdicional;
            e.Graphics.DrawString(textoTotalPesoMaisComprado, contatoFonte, Brushes.Black, new Point(margemInterna, linhaAtual));
            linhaAtual += alturaTextoAdicional;
            linhaAtual += alturaTextoAdicional;
        ...
        private void btnConsultarData_Click(object sender, EventArgs e)
        {
            ...
            string materialSelecionado = listaMateriais.SelectedItem as string;
            DateTime dataSelecionada = dateTimePickerDia.Value.Date; // Obtém a data do DateTimePicker sem a parte da hora
            ...
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                try
                {
                    ...
                    // Adiciona filtro pela data selecionada
                    query += " AND DATE(dataHoraAtual) = @dataSelecionada";
                    ...
                    // Atualizar label lbTotalPesoMaterialMaisComprado diretamente
                    lbTotalPesoMaterialMaisComprado.Text = pesoMaisComprado.ToString("N3") + " kg";
                }
                catch (Exception ex)
                {
                    MessageBox.Show($"Erro ao executar a consulta: {ex.Message}", "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        // Remova qualquer outra definição do método CalcularSomaTotalPesoMaterialMaisComprado que possa existir no restante do código.
