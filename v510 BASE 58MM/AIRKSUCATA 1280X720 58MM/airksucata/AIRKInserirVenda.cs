...
{
    public partial class AIRKInserirVenda : Form
    {
        ...
        {
            string connectionString = "Server=localhost;Database=xlata_db;User=root;Password=;";
            string query = "SELECT id_venda FROM vendas WHERE id_venda = @NumeroUnico";
        ...
        private decimal CalcularValorTotal()
        {
            ...
            {
                // Obtenha a quantidade do campo txtQuant
        ...
        private void btnFinalizarCompra_Click(object sender, EventArgs e)
        {
            ...
            int quantidadeVenda = int.Parse(txtQuant.Text);
            ...
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                ...
                string query = "INSERT INTO vendas (produto_venda, valor_unit, valor_venda, forma_pagamento, quantidade_venda, dataVenda) " +
                    "VALUES (@Produto, @ValorUnit, @ValorVenda, @FormaPagamento, @Quantidade, @DataVenda)";

                using (MySqlCommand cmd = new MySqlCommand(query, connection))
                {
                    // Adicione parâmetros ao comando SQL
                    cmd.Parameters.AddWithValue("@Produto", nomeMaterial);
                    cmd.Parameters.AddWithValue("@ValorUnit", valorVenda);
                    cmd.Parameters.AddWithValue("@ValorVenda", valorUnitario);
        ...
        private void SalvarEmArquivoJson(string nomeMaterial, decimal valorVenda, decimal valorUnitario, string formaPagamento, int quantidadeVenda, DateTime dataVenda)
        {
            // Crie um objeto para armazenar os dados
            var dadosVendaItem = new
            {
                Produto = nomeMaterial,
                ValorUnit = valorVenda.ToString("C"), // Formatando como moeda
            ...
            string nomePasta = $"relatorioVendaPorUnid\\{dataVenda:yyyy-MM-dd}";
            ...
            string nomeArquivoItem = $"AIRK_vendaunid_{dataVenda:HH.mm.ss}.json";
        ...
        private void VerificarHabilitacaoBtnFinalizar()
        {
            ...
            bool quantidadePreenchida = !string.IsNullOrEmpty(txtQuant.Text) && int.TryParse(txtQuant.Text, out int quantidade) && quantidade > 0;
