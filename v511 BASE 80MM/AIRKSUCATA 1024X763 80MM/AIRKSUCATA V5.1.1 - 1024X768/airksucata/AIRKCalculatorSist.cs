...
namespace airksucata
{
...

    {
        ...
        private void InitializeListView()
        {
            ...
            listRegistros.Columns.Add("ID", 50, HorizontalAlignment.Left);
            listRegistros.Columns.Add("Material", 140, HorizontalAlignment.Left);
            listRegistros.Columns.Add("Valor KG", 50, HorizontalAlignment.Left);
            listRegistros.Columns.Add("Peso", 75, HorizontalAlignment.Left);
            listRegistros.Columns.Add("Total", 80, HorizontalAlignment.Left);
            listRegistros.Columns.Add("Data e Hora", 140, HorizontalAlignment.Left);
        ...
        {
            string consulta = "SELECT id_registro, materialNome, materialValor, inputValor, totalPagar, dataHoraAtual FROM pedidos_impressos ORDER BY id_registro DESC LIMIT 5"; // Substitua por sua consulta SQL
        ...
        private void CreateMaterialButton(int row, int col, string nome)
        {
            ...
            newButton.Width = 127; // Defina a largura desejada
            newButton.Height = 35; // Defina a altura desejada
            newButton.BackColor = Color.FromArgb(0, 140, 0); // Cor de fundo
            newButton.ForeColor = Color.White; // Cor do texto
            newButton.Font = new Font("Arial", 7, FontStyle.Bold); // Fonte e tamanho do texto
        ...
        private void btnCe_Click(object sender, EventArgs e)
        {
            if (txtPeso.Text.Length > 0)
            {
                ...
                if (txtPeso.Text.Length == 0)
                {
                    ...
                    if (lbQuantRegistros.Text.Length > 0)
                    {
                        btnImprimir.Enabled = true; //DESATIVA O BOTÃO IMPRIMIR
                    }
                }
            }
            verificaInserirNome();
        ...
        public void statusSistema()
        {
            ...
            // Encontrar todos os arquivos no diretório com o padrão de nome "_*.arq"
            string[] arquivos = Directory.GetFiles(diretorio, "airkdigitalwpp11952785087direitosreservadosmarcaregistrada*.arq");
        ...
        private void formSistema_Load(object sender, EventArgs e)
        {
            ...
            listaRegistrosListView.Font = new Font("Arial", 11);
